...
var argv = _optimist2.default.usage('Generate a GraphQL Server from a schema types file.\n' + 'See http://graphql.org/docs/typesystem/ for more info.').demand('t').alias('t', 'typedef-file').describe('t', 'Typef file name').alias('q', 'query-name').describe('q', 'Name of root query type').default('q', 'RootQuery').alias('m', 'mutation-name').describe('m', 'Name of root mutation type').default('m', 'RootMutation').boolean('r').alias('r', 'relay').describe('r', 'include Relay types and interfaces(coming soon)').argv;

if (!_fs2.default.existsSync(argv.t)) {
  console.warn('Schema types file "' + argv.t + '" does not exist.');
  process.exit(1);
}
var typeString = _fs2.default.readFileSync(argv.t, 'utf8');

console.log('types ->', typeString);
